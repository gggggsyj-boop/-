...
class RagAgentService:
    ...
    def get_session_history(self, session_id: str) -> list:
        ...
        try:
            ...
            # 兼容不同版本 LangGraph 的返回值:
            # - MemorySaver.get(config) 返回 checkpoint dict
            # - get_tuple(config) 或旧实现可能返回带 checkpoint 属性的对象
            # - 少数旧版本可能返回 tuple，第一个元素是 checkpoint
            if hasattr(checkpoint_data, "checkpoint"):
                checkpoint_data = checkpoint_data.checkpoint  # type: ignore[attr-defined]
            elif isinstance(checkpoint_data, tuple):
                checkpoint_data = checkpoint_data[0] if checkpoint_data else {}
            ...
            history = []
            for msg in messages:
                # 只返回真正的对话消息；跳过系统提示、工具调用结果等内部消息。
                msg_type = getattr(msg, "type", None)
                if isinstance(msg, SystemMessage) or msg_type in {"system", "tool"}:
                    continue
